...
def mostrar_login():
    ...
    with col1:
        st.image("Fotos/cacif.png", use_container_width=True)
    with col2:
        st.image("Fotos/bandera.png", use_container_width=True)
    with col3:
        st.image("Fotos/improgress.png", use_container_width=True)

    st.markdown("<hr class='divider'>", unsafe_allow_html=True)
    st.markdown("<div class='login-title'>Observatorio de Ejecución Presupuestaria ​y Actores Políticos</div>", unsafe_allow_html=True)
    st.markdown(
...
# Si no ha iniciado sesión, mostrar login y detener la app aquí
...
# CONFIG
# =========================
FILE_PATH = "Data/snip_proyectos_clean.parquet"
FILE_PATH_2 = "Data/projects_clean.csv"
...
def load_data(path: str) -> pd.DataFrame:
    ...
    # snip y ejercicio: convertir a entero sin decimales (Int64 soporta NaN)
    ...
    text_cols = [
        ...
        "alcalde_ganador", "siglas_ganadora",
    ...
    # columnas numéricas
    num_cols = [
        "ejercicio", "monto_solicitado", "monto_inicial", "monto_vigente", "monto_ejecutado",
    ...
    # alias de columnas precomputadas en preprocessing
...
def apply_filters(df: pd.DataFrame) -> pd.DataFrame:
    ...
    instituciones = sorted(df_f["institucion"].dropna().unique().tolist())
    inst_sel = st.sidebar.multiselect("Institución", instituciones)
    if inst_sel:
        df_f = df_f[df_f["institucion"].isin(inst_sel)]

    min_year = int(df["ejercicio"].min())
    max_year = int(df["ejercicio"].max())
    year_range = st.sidebar.slider("Rango de años", min_year, max_year, (min_year, max_year))
    df_f = df_f[df_f["ejercicio"].between(year_range[0], year_range[1])]
...
def highlight_flags(row):
    if row.get("sin_meta_ejecutada_con_gasto") == "Sí":
        return ["background-color: #fde2e2"] * len(row)
    if row.get("sospechoso") == "Sí":
        return ["background-color: #f8d7da"] * len(row)
    return [""] * len(row)


def highlight_flags_full(row):
    if row.get("sin_meta_ejecutada_con_gasto") == "Sí":
        return ["background-color: #fde2e2"] * len(row)
    if row.get("sospechoso") == "Sí":
        return ["background-color: #f8d7da"] * len(row)
    if row.get("sobreejecucion_financiera") == "Sí":
...
def load_combined(path1: str, path2: str) -> pd.DataFrame:
    ...
    return pd.concat([df1, df2_nuevos], ignore_index=True)

df = load_combined(FILE_PATH, FILE_PATH_2)
df_filtered = apply_filters(df)

# ── Flags de riesgo (se calculan una sola vez sobre df_filtered) ──────────────
df_filtered["diferencia"] = df_filtered["monto_adjudicado"] - df_filtered["monto_ejecutado"]
df_filtered["sospechoso"] = np.where(
    df_filtered["monto_adjudicado"].notna()
    & (df_filtered["diferencia"].abs() < 1000)
    & (df_filtered["ratio_meta_ejecucion"] < 0.90),
    1, 0
)
df_filtered["sin_meta_ejecutada_con_gasto"] = np.where(
    ((df_filtered["meta_ejecutada"] == 0) | df_filtered["meta_ejecutada"].isna())
    & (df_filtered["monto_ejecutado"] > 0),
    1, 0
)
df_filtered["sobreejecucion_financiera"] = np.where(
    df_filtered["monto_adjudicado"].notna()
    & (df_filtered["monto_ejecutado"] > df_filtered["monto_adjudicado"]),
    1, 0
)
# ─────────────────────────────────────────────────────────────────────────────
...
st.title("Observatorio de Ejecución Presupuestaria ​y Actores Políticos")
...
with tab1:
    ...
    if municipio_sel:
        ...
        detalle_municipio["sospechoso"] = detalle_municipio["sospechoso"].map({1: "Sí", 0: "No"})
        detalle_municipio["sin_meta_ejecutada_con_gasto"] = detalle_municipio["sin_meta_ejecutada_con_gasto"].map({1: "Sí", 0: "No"})
        ...
        styled_df = (
            ...
            .format(
            {
                "snip": lambda x: "" if pd.isna(x) else f"{int(x)}",
                "monto_adjudicado": "{:,.2f}",
                "monto_ejecutado": "{:,.2f}",
...
with tab2:
   ...
   # if "promedio_ratio_meta_ejecutada" in mayor_one_display.columns:
    ...
    if alcalde_sel:
        ...
        detalle_alcalde["sospechoso"] = detalle_alcalde["sospechoso"].map({1: "Sí", 0: "No"})
        detalle_alcalde["sin_meta_ejecutada_con_gasto"] = detalle_alcalde["sin_meta_ejecutada_con_gasto"].map({1: "Sí", 0: "No"})
        ...
        styled_df = (
            ...
            .format(
                {
                    "snip": lambda x: "" if pd.isna(x) else f"{int(x)}",
                    "ejercicio": lambda x: "" if pd.isna(x) else f"{int(x)}",
                    "monto_adjudicado": "{:,.2f}",
                    "monto_ejecutado": "{:,.2f}",
                    "brecha_adjudicado_ejecutado": "{:,.2f}",
                    "meta_ejecutada": "{:,.2f}",
...
with tab3:
    ...
    # -------------------------
    # MAPA 3: PROYECTOS SIN META EJECUTADA CON GASTO
    # -------------------------
    st.subheader("Mapa de proyectos sin meta ejecutada pero con gasto")

    df_meta0_gasto = df_sos[df_sos["sin_meta_ejecutada_con_gasto"] == 1].copy()

    if not df_meta0_gasto.empty:
        total_muni_meta0 = (
            df_sos.groupby("municipio")["snip"]
            .nunique()
            .reset_index(name="total_proyectos")
        ...
        mapa_meta0 = (
            df_meta0_gasto.groupby("municipio")["snip"]
            .nunique()
            .reset_index(name="num_meta0_gasto")
        )

        mapa_meta0 = mapa_meta0.merge(total_muni_meta0, on="municipio", how="left")
        mapa_meta0["pct_meta0_gasto"] = mapa_meta0["num_meta0_gasto"] / mapa_meta0["total_proyectos"]
        mapa_meta0["municipio"] = mapa_meta0["municipio"].astype(str).str.strip().str.upper()

        color_col = "num_meta0_gasto" if modo_mapa == "Número" else "pct_meta0_gasto"
        colorbar_title = "Proyectos meta=0 con gasto" if modo_mapa == "Número" else "% proyectos meta=0 con gasto"
        ...
        st.warning("⚠️ No existen proyectos sin meta ejecutada con gasto para mostrar en el mapa.")
    ...
    for col in ["snip", "ejercicio"]:
        if col in tabla_display.columns:
            tabla_display[col] = tabla_display[col].map(lambda x: "" if pd.isna(x) else f"{int(x)}")
    for col in ["monto_adjudicado", "monto_ejecutado", "brecha_adjudicado_ejecutado", "meta_fisica", "meta_ejecutada"]:
        if col in tabla_display.columns:
    ...
    styled_df = (
        tabla_display.style
        .apply(highlight_flags_full, axis=1)
    )
...
with tab4:
    ...
    df_partidos_insights = df_filtered
    ...
    if partido_sel:
        ...
        # convertir flags 0/1 a "Sí"/"No" para display
        df_p = df_p.copy()
        df_p["sospechoso"] = np.where(df_p["sospechoso"] == 1, "Sí", "No")
        df_p["sin_meta_ejecutada_con_gasto"] = np.where(df_p["sin_meta_ejecutada_con_gasto"] == 1, "Sí", "No")
        df_p["sobreejecucion_financiera"] = np.where(df_p["sobreejecucion_financiera"] == 1, "Sí", "No")
        ...
        meta0_gasto = df_p[df_p["sin_meta_ejecutada_con_gasto"] == "Sí"]["snip"].nunique()
        ...
        detalle["orden_meta0"] = np.where(detalle["sin_meta_ejecutada_con_gasto"] == "Sí", 0, 1)
        ...
        styled = (
            detalle.style
            .apply(highlight_flags_full, axis=1)
            .hide(axis="columns", subset=["sospechoso", "sin_meta_ejecutada_con_gasto", "sobreejecucion_financiera"])
            .format(
            {
                "snip": lambda x: "" if pd.isna(x) else f"{int(x)}",
                "monto_adjudicado": "{:,.2f}",
        ...
        timeline = (
            alcaldes_por_anio
            .merge(monto_por_anio, on="ejercicio", how="left")
...
with tab5:
    ...
    meta0_prov = (
        df_tmp[df_tmp["sin_meta_ejecutada_con_gasto"] == 1]
        .groupby("proveedor")["snip"]
        .nunique()
    ...
    agg_prov["ratio_sin_meta_ejecutada_con_gasto"] = agg_prov["casos_sin_meta_ejecutada_con_gasto"] / agg_prov["proyectos"]
    ...
    ratio_meta0 = agg_prov[agg_prov["proyectos"] >= 3].sort_values(
        ["casos_sin_meta_ejecutada_con_gasto", "ratio_sin_meta_ejecutada_con_gasto"], ascending=[False, False]
    )
    if not ratio_meta0.empty:
        top_meta0 = ratio_meta0.iloc[0]
        top_meta0_text = (
            f"{top_meta0['proveedor']} "
            f"({top_meta0['casos_sin_meta_ejecutada_con_gasto']:,.0f} — {top_meta0['ratio_sin_meta_ejecutada_con_gasto']:.2%})"
        )
   ...
   # base principal por proveedor
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "casos_sin_meta_ejecutada_con_gasto": st.column_config.NumberColumn(
    ...
    if proveedor_sel:
        ...
        detalle_proveedor["sospechoso"] = detalle_proveedor["sospechoso"].map({1: "Sí", 0: "No"})
        detalle_proveedor["sin_meta_ejecutada_con_gasto"] = detalle_proveedor["sin_meta_ejecutada_con_gasto"].map({1: "Sí", 0: "No"})
        detalle_proveedor["sobreejecucion_financiera"] = detalle_proveedor["sobreejecucion_financiera"].map({1: "Sí", 0: "No"})
        ...
        proyectos_sin_meta_ejecutada_con_gasto = (
            (detalle_proveedor["sin_meta_ejecutada_con_gasto"] == "Sí").sum()
            if "sin_meta_ejecutada_con_gasto" in detalle_proveedor.columns else 0
        )
        proyectos_sobreejecucion = (
        ...
        min_year = detalle_proveedor["ejercicio"].min() if "ejercicio" in detalle_proveedor.columns else np.nan
        max_year = detalle_proveedor["ejercicio"].max() if "ejercicio" in detalle_proveedor.columns else np.nan
        ...
        k6.metric("Promedio ratio meta ejecutada", f"{ratio_promedio:.2%}" if pd.notna(ratio_promedio) else "")
        k7.metric("Meta ejecutada = 0 con gasto", f"{proyectos_sin_meta_ejecutada_con_gasto:,.0f}")
        ...
        detalle_proveedor["orden_meta0"] = np.where(detalle_proveedor["sin_meta_ejecutada_con_gasto"] == "Sí", 0, 1)
        ...
        detalle_proveedor = detalle_proveedor.sort_values(
            ...
            ascending=[True, True, True, False]
        ).drop(columns=["orden_meta0", "orden_sospechoso", "orden_sobre", "meta_ejecutada"])
        ...
        df_total_anual = (
            detalle_proveedor.groupby(["ejercicio"])["monto_ejecutado"]
            .sum()
            .reset_index()
            .sort_values("ejercicio")
        )
        ...
        fig_general = px.line(
            ...
            labels={"monto_ejecutado": "Monto Total (Q)", "ejercicio": "Año"},
        ...
        with st.expander("Ver detalle desglosado por Municipio y Alcalde"):
            ...
            df_faceta = (
            detalle_proveedor.groupby(["municipio", "ejercicio", "alcalde", "partido"])["monto_ejecutado"]
            .sum()
            .reset_index()
            ...
            category_orders={"ejercicio": sorted(df_faceta["ejercicio"].unique())},
            title=f"Evolución por Municipio y Gestión: {proveedor_sel}",
            labels={"monto_ejecutado": "Monto (Q)", "ejercicio": "Año"},
